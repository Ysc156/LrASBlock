...
#     x = SA.create_mem_sh(name, var.shape)
#     x[...] = var[...]
...
def data_prepare_semantic_kitti(coord, feat, label, split='train', voxel_size=0.04, voxel_max=None, transform=None, shuffle_index=False):
    cood_int_0 = coord.copy()
    if transform:
        # coord, feat, label = transform(coord, feat, label)
        coord, _ = transform(coord, coord)
    if voxel_size:
        cood_int = coord.copy()
        coord_min = np.min(coord, 0)
        coord -= coord_min
        uniq_idx = voxelize(coord, voxel_size)
        while len(uniq_idx)==1:
            coord, _ = transform(cood_int_0, cood_int_0)
            coord_min = np.min(coord, 0)
            coord -= coord_min
            uniq_idx = voxelize(coord, voxel_size)
        coord, feat, label = coord[uniq_idx], feat[uniq_idx], label[uniq_idx]
