...
from .skip_connection_new_copy import LrASBlockv1, LrASBlockv2
import random
...
class Upsample(nn.Module):
    ...
    def forward(self, feats, xyz, support_xyz, offset, support_offset, support_feats=None, skip_feat=None):
...
class Stratified(nn.Module):
    def __init__(self, downsample_scale, depths, channels, num_heads, window_size, up_k, \
                 ...
                 num_layers=4, fea_dim=6, num_classes=13, ratio=0.25, k=16, prev_grid_size=0.04, sigma=1.0,
        ...
        self.PCT_skip = nn.ModuleList([LrASBlockv2([channels[0]], channels[i]) for i in range(num_layers - 2, -1, -1)])

        # self.PCT_skip_scale_gate = nn.ParameterList([nn.Parameter(torch.randn(128)) for _ in range(3)])
        ...
        self.Global_atten_points_skip =  nn.ModuleList([Global_atten_points(channels[i]) for i in range(len(channels)-1)])
        ...
        self.init_weights()
        self.Global_skip = LrASBlockv1(channels[:1], channels[0])
    def forward(self, feats, xyz, offset, batch, neighbor_idx):
        ...
        xyz_sample, feat_sample, offset_sample = sample_global_points([xyz, feats, offset], 250)
        feat_ = []
        att_weight_G = []
        for j in range(len(offset_sample)):
            ...
            x_temp, weight_temp = self.Global_skip(feat_sample_j.unsqueeze(0).transpose(1, 2),\
                                            feat_j.unsqueeze(0).transpose(1, 2),\
                                            xyz_sample_j.unsqueeze(0).transpose(1, 2),\
                                            xyz_j.unsqueeze(0).transpose(1, 2))
            feat_.append(x_temp)
            att_weight_G.append(weight_temp)
            #print('feat_', feat_)
        Global_point = [xyz_sample, torch.cat(feat_, dim=2).squeeze(0).transpose(0,1), offset_sample]
        ...
        for i, upsample in enumerate(self.upsamples):
            ...
            feat_ = []
            xyz_sample = Global_point[0]
            feat_sample = Global_point[1]
            offset_sample = Global_point[2]
            # print("lenfeatlist{}".format(len(feat_list)))
            # print(len(offset_skip))
            att_weight = []
            for j in range(len(offset_skip)):
                ...
                x_temp, weight_temp = self.PCT_skip[i](feat_j.unsqueeze(0).transpose(1, 2), feat_sample_j.unsqueeze(0).transpose(1, 2), xyz_j.unsqueeze(0).transpose(1, 2), xyz_sample_j.unsqueeze(0).transpose(1, 2))
                feat_.append(x_temp)
                if i == len(self.upsamples)-1:
                    att_weight.append(weight_temp)
                                    #  feat_sample_j.unsqueeze(0).transpose(1, 2) * self.PCT_skip_scale_gate[i].unsqueeze(1).to(feat_sample_j.device),\
            feat_pop = torch.cat(feat_, dim=2).squeeze(0).transpose(0,1)
                #print('feat_', feat_)
            #print('feats:',feats)
            #print('feat_pop:',feat_pop)
...
class Global_atten_points(nn.Module):

    def __init__(self, channels):
        super().__init__()
        self.layers = nn.Sequential(nn.Linear(channels, channels), nn.Linear(channels, channels), nn.Linear(channels, channels))
        self.ReLU = nn.ReLU()
        self.G_N = nn.GroupNorm(8,channels)
    def forward(self, pxo, self_points):
        p, x, o = pxo  # (n, 3), (n, c), b
        ...
        for i in range(0, o.shape[0]-1):
            ...
            q = self.ReLU(self.G_N(self.layers[0](xso)))
            k = self.ReLU(self.G_N(self.layers[1](xo)))
            v = self.ReLU(self.G_N(self.layers[2](xo)))
