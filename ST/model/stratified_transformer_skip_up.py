...
from .skip_connection_new import PCT
import random
...
class Upsample(nn.Module):
    def __init__(self, k, in_channels, out_channels, skip_c, bn_momentum=0.02):
        super().__init__()
        self.k = k
        ...
        self.linear_skip = nn.ModuleList(
                        [nn.Sequential(nn.LayerNorm(c), nn.Linear(c, out_channels)) for c in skip_c]
                        )
        # self.linear_skip = nn.Sequential(nn.LayerNorm(skip_c), nn.Linear(skip_c, out_channels))
    def forward(self, feats, xyz, support_xyz, offset, support_offset, support_feats=None, global_points=None):
        feats = self.linear1(support_feats) + pointops.interpolation(xyz, support_xyz, self.linear2(feats), offset,
                                                                     support_offset)
                        # pointops.interpolation(xyz, global_points[0], self.linear_skip(global_points[1]), offset, global_points[2])
        assert len(self.linear_skip) == len(global_points)
        global_xyz = global_points[0][0]
        global_offset = global_points[0][2]
        # feat_global = []
        # for i in range(len(self.linear_skip)):
        #     feat_global.append(self.linear_skip[i](global_points[i][1]))
        # feat_global = torch.mean(torch.stack(feat_global), dim=0)
        # feats = feats + pointops.interpolation(global_xyz, support_xyz, feat_global, global_offset, support_offset, k=150)
        for i in range(len(self.linear_skip)):
            global_xyz = global_points[i][0]
            global_offset = global_points[i][2]
            
            if global_xyz is not None:
                feat_global = self.linear_skip[i](global_points[i][1])
                # print(global_xyz.shape[0]/len(global_offset))
                k = int(global_xyz.shape[0]/len(global_offset))
                feats = feats + pointops.interpolation(global_xyz, support_xyz, feat_global, global_offset, support_offset, k=k)
...
class Stratified(nn.Module):
    def __init__(self, downsample_scale, depths, channels, num_heads, window_size, up_k, \
                 ...
                 num_layers=4, fea_dim=None, num_classes=13, ratio=0.25, k=16, prev_grid_size=0.04, sigma=1.0,
        ...
        self.upsamples = nn.ModuleList(
            [Upsample(up_k, channels[i], channels[i - 1], channels) for i in range(num_layers - 1, 0, -1)])
        # self.PCT_skip = nn.ModuleList([PCT(channels[i], channels[i]) for i in range(num_layers - 2, -1, -1)])
        self.Global_skip = nn.ModuleList([PCT(channels[i], channels[i]) for i in range(len(channels))])
        # self.skip_scale_gate = [nn.Parameter(torch.randn(128)) for _ in range(3)]
        ...
        # self.PCT_skip_linear1 = nn.Sequential(nn.Linear(6, 128), nn.Linear(6, 128), nn.Linear(6, 128))
        
        #self.PCT_skip_linear0 = nn.Sequential(nn.Linear(6, 128), nn.Linear(6, 128), nn.Linear(128, 128))
        #self.PCT_skip_Pt = torch.nn.Parameter(torch.randn(150, 128))
        self.init_weights()
    ...
    def forward(self, feats, xyz, offset, batch, neighbor_idx):
        ...
        xyz_stack = []
        offset_stack = []
        global_num = [250,250,250,250, -1]
        # global_num = [150,50,100,200,250]
        # global_num = [50,100,200,250,-1]
        # global_num = [250,200,100,50,-1]
        ...
        if self.layer_start == 1:
            ...
            xyz_sample, feat_sample, offset_sample = sample_global_points([xyz, feats, offset], global_num[0])
            feat_ = []
            for j in range(len(offset_sample)):
                if j == 0:
                    feat_j = feats[:offset[j]]
                    xyz_j = xyz[:offset[j]]
                    feat_sample_j = feat_sample[:offset_sample[j]]
                    xyz_sample_j = xyz_sample[:offset_sample[j]]
                else:
                    feat_j = feats[offset[j - 1]:offset[j]]
                    xyz_j = xyz[offset[j - 1]:offset[j]]
                    feat_sample_j = feat_sample[offset_sample[j-1]:offset_sample[j]]
                    xyz_sample_j = xyz_sample[offset_sample[j-1]:offset_sample[j]]
                feat_.append(self.Global_skip[0](feat_j.unsqueeze(0).transpose(1, 2),\
                                    feat_sample_j.unsqueeze(0).transpose(1, 2),\
                                    xyz_j.unsqueeze(0).transpose(1, 2),\
                                    xyz_sample_j.unsqueeze(0).transpose(1, 2)))
                #print('feat_', feat_)
            feat_global_list.append([xyz_sample, torch.cat(feat_, dim=1).squeeze(0), offset_sample])
        ...
        for i, layer in enumerate(self.layers):
            ...
            if self.layer_start == 1:
                ...
                c = i
            xyz_sample, feat_sample, offset_sample = sample_global_points([xyz, feats, offset], global_num[i])
            
            assert len(offset_sample) == len(offset)
            feat_ = []
            if xyz_sample is not None:
                for j in range(len(offset_sample)):
                    if j == 0:
                        ...
                        feat_sample_j = feat_sample[offset_sample[j-1]:offset_sample[j]]
                        xyz_sample_j = xyz_sample[offset_sample[j-1]:offset_sample[j]]
                    feat_.append(self.Global_skip[c](feat_j.unsqueeze(0).transpose(1, 2),\
                                        feat_sample_j.unsqueeze(0).transpose(1, 2),\
                                        xyz_j.unsqueeze(0).transpose(1, 2),\
                                        xyz_sample_j.unsqueeze(0).transpose(1, 2)))
                    #print('feat_', feat_)
                ...
                feat_global_list.append([None, None, offset_sample])
            feats_stack.append(feats)
            xyz_stack.append(xyz)
            offset_stack.append(offset)
        ...
        for i, upsample in enumerate(self.upsamples):
            feats, xyz, offset = upsample(feats, xyz, xyz_stack.pop(), offset, offset_stack.pop(),
                                          support_feats=feats_stack.pop(), global_points=feat_global_list)
            #print(i, feats)
...
def collect_learnable_points(pxo, self_points, layers):
    p, x, o = pxo  # (n, 3), (n, c), b
    ...
    for i in range(0, o.shape[0]-1):
        ...
        q = layers[2](self_points)
        k = layers[0](xo)
        v = layers[1](xo)
        W_h = torch.matmul(q, k.transpose(0,1))/(q.shape[1]**0.5)  # (S, n_0)
        np = torch.matmul(F.softmax(W_h, dim=-1), po)  # (S, 3)
        gamma = 16
        #dist_w_norm = torch.norm((torch.max(po.squeeze(0),0)[0] - torch.min(po.squeeze(0),0)[0]), 2)/20
        #dist = square_distance_norm(np,po)/dist_w_norm
        #W_g = 0.5**dist  # (S, n)
        W_g = torch.exp(-gamma*(square_distance_norm(np,po)))
        W = W_g * W_h  # (S, n)
        nx = torch.matmul(F.softmax(W, dim=-1), v)  # (S, c)
...
def collect_global_points(pxo, self_points, layers):
    p, x, o = pxo  # (n, 3), (n, c), b
    ps, xs, os = self_points
    o = torch.cat([o[0:1]*0, o], dim=0)
    os = torch.cat([os[0:1]*0, os], dim=0)
    #n_S = self_points[0].shape[0]
    n_x = None
    ...
    for i in range(0, o.shape[0]-1):
        ...
        W = torch.matmul(q, k.transpose(0,1))/(q.shape[1]**0.5)  # (S, n_0)
        dist_w_norm = torch.norm((torch.max(po,0)[0] - torch.min(po,0)[0]), 2)/20
        dist_w = square_distance_norm(pso,po)//dist_w_norm
        w_d = 0.5**dist_w
        #W_g = torch.exp(-gamma*(square_distance_norm(np,po)))  # (S, n)
        #W = W_g * W_h  # (S, n)
        nx = torch.matmul(F.softmax(W, dim=-1)*w_d, v)  # (S, c)
