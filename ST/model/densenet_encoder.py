...
class Stratified(nn.Module):
    ...
    def forward(self, feats, xyz, offset, batch, neighbor_idx):
        ...
        for i, layer in enumerate(self.layers):
            ...
            for i in range(len(feats_stack)):
                n_offset = torch.cuda.IntTensor(offset)
                idx = pointops.furthestsampling(xyz_stack[i], offset_stack[i], n_offset)  # (m)
                ft = feats_stack[i][idx.long(), :]  # (m, 3)

                ft = ft.repeat(1, feats.shape[1]//ft.shape[1])
                feats = feats + ft
            feats_stack.append(feats)
            xyz_stack.append(xyz)
            offset_stack.append(offset)
