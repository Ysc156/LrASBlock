...
class Stratified(nn.Module):
    ...
    def forward(self, feats, xyz, offset, batch, neighbor_idx):
        ...
        if is_FPS == 1:
            ...
            Global_point = sample_global_points([xyz, feats, offset], 150)
        
            xyz_sample, feat_sample, offset_sample = collect_global_points([xyz, feats, offset], Global_point, self.PCT_skip_linear1)
        
        elif is_FPS == 0:
            xyz_sample, feat_sample, offset_sample = collect_learnable_points([xyz, feats, offset], self.PCT_skip_Pt, self.PCT_skip_linear0)
        else:
            Global_point = sample_global_points([xyz, feats, offset], 150)
            xyz_sample_1, feat_sample_1, offset_sample_1 = collect_global_points([xyz, feats, offset], Global_point, self.PCT_skip_linear1)
            xyz_sample_0, feat_sample_0, offset_sample_0 = collect_learnable_points([xyz, feats, offset], self.PCT_skip_Pt, self.PCT_skip_linear0)
            offset_sample = offset_sample_1.clone()
            for j in range(len(offset_sample_1)):
                if j == 0:
                    p1 = xyz_sample_1[:offset_sample_1[j]]
                    p0 = xyz_sample_0[:offset_sample_0[j]]
                    x1 = feat_sample_1[:offset_sample_1[j]]
                    x0 = feat_sample_0[:offset_sample_0[j]]
                    xyz_sample = torch.cat([p1, p0], 0)
                    feat_sample = torch.cat([x1, x0], 0)
                else:
                    p1 = xyz_sample_1[offset_sample_1[j-1]:offset_sample_1[j]]
                    p0 = xyz_sample_0[offset_sample_0[j-1]:offset_sample_0[j]]
                    x1 = feat_sample_1[offset_sample_1[j-1]:offset_sample_1[j]]
                    x0 = feat_sample_0[offset_sample_0[j-1]:offset_sample_0[j]]
                    xyz_sample = torch.cat([xyz_sample, p0], 0)
                    xyz_sample = torch.cat([xyz_sample, p1], 0)
                    feat_sample = torch.cat([feat_sample, x0], 0)
                    feat_sample = torch.cat([feat_sample, x1], 0)
                offset_sample[j] = offset_sample[j]+offset_sample_0[j]
                
            
        #Get sparse global feats and xyz
        
        #print('feat_sample', feat_sample.shape)
        #print('xyz_sample', xyz_sample.shape)
        ...
        for i, upsample in enumerate(self.upsamples):
            ...
            for j in range(len(offset_skip)):
                ...
                #print(j)
                #print('feat_j', feat_j)
                # print('feat_sample_j', feat_sample_j.shape)
                # print(self.layer_scale_gate.shape)
                feat_.append(
