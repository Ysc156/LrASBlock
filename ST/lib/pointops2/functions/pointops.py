...
def interpolation_v2(xyz, new_xyz, feat, offset, new_offset, k=3):
    ...
    total_rows = new_offset[-1].item()
    row_length = offset[0]

    # 初始化一个空张量
    idx_ = torch.empty((total_rows, row_length), dtype=torch.int32)

    # 计算每组的起始值，并填充 tensor
    start = 0
    for i, end in enumerate(new_offset):
        # 每个组的行范围
        group_range = torch.arange(i * row_length, (i + 1) * row_length, dtype=torch.int32)
        # 将 group_range 复制到对应的行
        idx_[start:end] = group_range.unsqueeze(0).repeat(end - start, 1)
        # 更新起始行
        start = end
    ...
    # idx = torch.arange(k, dtype=torch.int32, device=xyz.device).unsqueeze(0).repeat(8128, 1)
    dist = torch.sqrt(((new_xyz.unsqueeze(1) - xyz[idx_.long()]) ** 2).sum(-1) + 1e-8)
    ...
    new_feat = torch.cuda.FloatTensor(new_xyz.shape[0], feat.shape[1]).zero_()
    for i in range(k):
        new_feat += feat[idx_[:, i].long(), :] * weight[:, i].unsqueeze(-1)
    return new_feat
