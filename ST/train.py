...
from util.semantic_kitti import semantic_kitti
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    if args.arch == 'stratified_transformer':
        ...
        from model.densenet_encoder import Stratified
    ...
    elif args.arch == 'swin3d_transformer_skip':

        from model.swin3d_transformer_skip import Swin
    ...
    if args.optimizer == 'SGD':
        ...
        param_dicts = [
            {
                "params": [p for n, p in model.named_parameters() if "blocks" not in n and p.requires_grad and "skip" not in n],
                "lr": args.base_lr,
            },
            {
                "params": [p for n, p in model.named_parameters() if "blocks" in n and p.requires_grad and "skip" not in n],
                "lr": args.base_lr * 0.1,
            },
            {  
                "params": [p for n, p in model.named_parameters() if "skip" in n],
                "lr": args.base_lr * 1,
                }
        ]
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            if switch == 0:
                ...
                #    new_state_dict[k.replace("item", "stem")] = v
                ...
                best_iou = 0                        
            if main_process():
                logger.info("=> loaded checkpoint '{}' (epoch {}), best_iou is {}".format(args.resume, checkpoint['epoch'], checkpoint['best_iou']))
        else:
            if main_process():
    ...
    if args.data_name == 's3dis':
        ...
        train_split = args.get("train_split", "train")
        if main_process():
            logger.info("kitti. train_split: {}".format(train_split))

        train_data = semantic_kitti(split=train_split, data_root=args.data_root, voxel_size=args.voxel_size,
                               voxel_max=args.voxel_max, transform=train_transform, shuffle_index=True, loop=args.loop)
    else:
    ...
    if args.data_name == 's3dis':
        ...
        val_data = semantic_kitti(split='val', data_root=args.data_root, voxel_size=args.voxel_size, voxel_max=800000,
                             transform=val_transform)
    else:
        raise ValueError("The dataset {} is not supported.".format(args.data_name))
    ...
    if args.scheduler == "MultiStepWithWarmup":
        ...
        if switch == 0:
            ...
            milestones = [int(x) for x in args.milestones.split(",")] if hasattr(args, "milestones") else [
                int(args.epochs * 0.5), int(args.epochs * 0.6)]
    ...
    if args.resume and os.path.isfile(args.resume) and not switch:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.evaluate and (epoch_log % args.eval_freq == 0):
            ...
            if main_process():
                ...
                logger.info('best_iou is {}'.format(best_iou))
        ...
        if (epoch_log % args.save_freq == 0) and main_process():
            ...
            if is_best:
                best_iou = mIoU_val
                logger.info('Best miou is update to %.3f' % (best_iou))
...
def train(train_loader, model, criterion, optimizer, epoch, scaler, scheduler):
    ...
    for i, (coord, feat, target, offset) in enumerate(train_loader):  # (n, 3), (n, c), (n), (b)
        ...
        for param_group in optimizer.param_groups:
                lr = max(param_group['lr'], LEARNING_RATE_CLIP)
                param_group['lr'] = lr
